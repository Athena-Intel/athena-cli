// Auto-generated by @fern-api/cli-generator's copySpecs step.
// Edit the SDK template / generator if you need to change the shape.

mod custom;
mod sdk;

use fern_cli_sdk::app::CliApp;
use fern_cli_sdk::openapi::OpenApiBinding;
use fern_cli_sdk::auth::{ApiKeyAuth, AuthCredentialSource};

fn main() {
    let app = CliApp::new("athena-intelligence-api")
        .auth(ApiKeyAuth::new("APIKeyHeader").source(AuthCredentialSource::any(vec![AuthCredentialSource::cli("api-key"), AuthCredentialSource::from_env("ATHENA_INTELLIGENCE_API_API_KEY")])))
        .binding(
            OpenApiBinding::new()
                .spec(include_str!("openapi0.json"))
        );

    let app = custom::register(app);

    app.run()
}
