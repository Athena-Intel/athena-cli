...
[![crates.io shield](https://img.shields.io/crates/v/athena_sdk)](https://crates.io/crates/athena_sdk)
...
```rust
match client.agents.invoke_by_id(None)?.await {
    Ok(response) => {
...
let request = CustomAgentRequest {
    ...
};
```
...
```rust
let response = client.agents.invoke_by_id(
