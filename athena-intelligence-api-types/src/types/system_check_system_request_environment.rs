pub use crate::prelude::*;
#[allow(unused_imports)]
use super::*;

/// The spec environment: development (the default) or production. Health and incidents are kept per environment and never mix.
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum CheckSystemRequestEnvironment {
    Development,
    Production,
    /// This variant is used for forward compatibility.
    /// If the server sends a value not recognized by the current SDK version,
    /// it will be captured here with the raw string value.
    __Unknown(String),
}
impl Serialize for CheckSystemRequestEnvironment {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self {
            Self::Development => serializer.serialize_str("development"),
            Self::Production => serializer.serialize_str("production"),
            Self::__Unknown(val) => serializer.serialize_str(val),
        }
    }
}

impl<'de> Deserialize<'de> for CheckSystemRequestEnvironment {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = String::deserialize(deserializer)?;
        match value.as_str() {
            "development" => Ok(Self::Development),
            "production" => Ok(Self::Production),
            _ => Ok(Self::__Unknown(value)),
        }
    }
}

impl fmt::Display for CheckSystemRequestEnvironment {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Development => write!(f, "development"),
            Self::Production => write!(f, "production"),
            Self::__Unknown(val) => write!(f, "{}", val),
        }
    }
}
