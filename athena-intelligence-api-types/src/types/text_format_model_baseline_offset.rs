pub use crate::prelude::*;
#[allow(unused_imports)]
use super::*;

#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum TextFormatModelBaselineOffset {
    Superscript,
    Subscript,
    Baseline,
    /// This variant is used for forward compatibility.
    /// If the server sends a value not recognized by the current SDK version,
    /// it will be captured here with the raw string value.
    __Unknown(String),
}
impl Serialize for TextFormatModelBaselineOffset {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self {
            Self::Superscript => serializer.serialize_str("superscript"),
            Self::Subscript => serializer.serialize_str("subscript"),
            Self::Baseline => serializer.serialize_str("baseline"),
            Self::__Unknown(val) => serializer.serialize_str(val),
        }
    }
}

impl<'de> Deserialize<'de> for TextFormatModelBaselineOffset {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = String::deserialize(deserializer)?;
        match value.as_str() {
            "superscript" => Ok(Self::Superscript),
            "subscript" => Ok(Self::Subscript),
            "baseline" => Ok(Self::Baseline),
            _ => Ok(Self::__Unknown(value)),
        }
    }
}

impl fmt::Display for TextFormatModelBaselineOffset {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Superscript => write!(f, "superscript"),
            Self::Subscript => write!(f, "subscript"),
            Self::Baseline => write!(f, "baseline"),
            Self::__Unknown(val) => write!(f, "{}", val),
        }
    }
}
